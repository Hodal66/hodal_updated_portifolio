    ...
    <section className={`min-h-screen px-4 sm:px-6 pt-28 pb-16 transition-all duration-700 ${isLoaded ? 'opacity-100' : 'opacity-0'}`}>
      <div className="max-w-4xl mx-auto">
        <div className="text-center mb-12">
          ...
          <p className={`max-w-xl mx-auto ${isDark ? 'text-gray-400' : 'text-slate-600'}`}>{t('experience.description')}</p>
        </div>
        ...
        <div className="space-y-8">
          {experience.map((exp, idx) => (
            <div key={idx} className="relative pl-8">
              ...
              <div className={`absolute -left-2 top-0 w-[18px] h-[18px] bg-primary-500 rounded-full border-4 shadow-lg shadow-primary-500/50 ${isDark ? 'border-dark-950' : 'border-slate-50'}`} />

              <div className={`ml-4 p-6 sm:p-8 rounded-2xl transition-all ${isDark ? 'card hover:border-primary-500/20' : 'bg-white border border-slate-200 shadow-sm hover:border-primary-500/30 hover:shadow-md'}`}>
                <div className="flex flex-wrap items-center gap-3 mb-4">
                  ...
                  <span className={`px-3 py-1.5 rounded-full text-sm ${isDark ? 'bg-white/5 text-gray-400' : 'bg-slate-100 text-slate-500'}`}>
                    {exp.type}
                  </span>
                ...
                <h3 className={`text-xl sm:text-2xl font-bold mb-1 ${isDark ? 'text-white' : 'text-slate-900'}`}>{exp.role}</h3>
                <p className="text-primary-400 font-medium mb-4">{exp.company}</p>
                <p className={`leading-relaxed mb-6 ${isDark ? 'text-gray-400' : 'text-slate-600'}`}>{exp.description}</p>

                <div>
                  <h4 className={`text-sm font-semibold mb-3 ${isDark ? 'text-gray-300' : 'text-slate-700'}`}>{t('experience.keyAchievements')}:</h4>
                  <div className="grid sm:grid-cols-2 gap-2">
                    {exp.achievements.map((achievement, i) => (
                      <div key={i} className="flex items-start gap-2 text-sm">
                        ...
                        <span className={isDark ? 'text-gray-400' : 'text-slate-600'}>{achievement}</span>
                      </div>
                    ))}
                  </div>
                </div>
